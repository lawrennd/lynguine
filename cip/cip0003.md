...
A workaround has been implemented instead of the proper architectural fix:
- referia's `update_name_column_map()` allows overwriting "default mappings" (identity and camelCase)
- This allows explicit interface mappings to override early identity mappings
- However, the timing issue remains: referia still calls `_augment_column_names()` in `__init__`

**Proper fix (as described in this CIP):**
- [ ] Phase 1: Move augmentation to from_flow in referia
- [ ] Phase 2: Verify no regressions
- [ ] Phase 3: Documentation

**Note**: This CIP requires changes to referia (separate repository). See referia's CIP-0005 for the corresponding implementation plan.
