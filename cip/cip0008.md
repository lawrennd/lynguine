...
### ✅ Applications That Benefit from Server Mode
...
- Short-lived processes (start → execute → exit)
- Stateless operations

**Examples**:
1. **lamd (primary use case)**:
   - Builds CVs using make with 38+ subprocess calls per build
   - Each `mdfield` call: new process → import lynguine → extract field → exit
   - Each `mdlist` call: new process → import lynguine → process data → exit
...
### ❌ Applications That DON'T Benefit from Server Mode
...
|-------------------|-----------------|-------------|-------------------|
| lamd (make/shell) | Many (38+/build) | ~72s/build | ✅ **210x faster** |
| CLI batch scripts | Many | High | ✅ **10-200x faster** |
| CI/CD pipelines | Multiple steps | High | ✅ **Significant** |
| referia (Jupyter) | One (kernel) | Amortized | ❌ **None** |
| Web apps | One (startup) | Amortized | ❌ **None** |
| Long Python scripts | One | Amortized | ❌ **None** |
...
**Client API** (mirrors lynguine's `DataObject` API):
```python
# Create session (loads interface file once)
session = client.create_session(interface_file='cv_config.yml')

# Focus-based navigation (like DataObject, for mdfield-style usage)
session.set_index('person_1')
session.set_column('name')
value = session.get_value()  # Transfer ~bytes (not full DataFrame)
...
**lamd Integration Example** (`mdfield` tool):
```python
# Current (stateless): 38 × 1.9s startup = 72s overhead
# With Phase 5 (stateful): 1 session + 38 lightweight ops = 1.9s

client = ServerClient(auto_start=True)
session = client.create_session(interface_file='neil-lawrence.yml')

# Extract 27 fields (minimal transfer per field)
fields = {}
for field in ['name', 'email', 'affiliation', 'title', ...]:
    session.set_column(field)
    fields[field] = session.get_value()  # ~bytes per field vs full DataFrame

# Improvement: 35-40x faster for lamd
```
...
- [ ] Focus-based navigation (set_index, set_column, get_value) - mirrors `DataObject` API
- [ ] Bulk operations (get_slice, get_column_data, filter)
- [ ] Series operations (set_selector, set_subindex, get_subseries)
- [ ] Session metadata: shape, columns, indices, memory usage, created time
