...
### Phase 1: Profiling (To Be Completed)

Before implementing, we need to measure:
...
1. **Startup Breakdown**:
   ...
   # Profile major imports
   import pandas  # How much?
   import numpy   # How much?
   # etc.
   ```

2. **Operation Timing**:
   - Time for simple file access
...
### Phase 2: Usage Pattern Analysis (To Be Completed)

Understand how lamd and other applications use lynguine:

1. **Call Frequency**:
   - How many lynguine calls per typical operation?
   - What's the time distribution?
   - Are calls sequential or could they be batched?

2. **Operation Types**:
   - What operations are most common?
   - Read-only or read-write?
   - Do operations share data/configuration?

3. **Performance Requirements**:
   - What's an acceptable startup time?
   - What's an acceptable per-operation time?
   - What's the performance improvement target?

### Expected Findings (Hypothesis)

**Hypothesis**: Import costs dominate startup time
- pandas/numpy imports: ~0.5-1.0s
- lynguine configuration parsing: ~0.1-0.2s
- Total startup: ~1-2s per call

**If true**: Server mode with persistent process could amortize this cost across many operations.
...
- [ ] Profile current startup costs (baseline measurements)
...
|----------|---------|--------|-------------|
| Single call | ~2s | ~2s | 1x (no change) |
| 10 calls | ~20s | ~3s | 6-7x faster |
| 100 calls | ~200s | ~12s | 15-20x faster |

**Measurements needed**:
- Startup time breakdown
- Per-operation time
- Server overhead per request
